...
const Cubes = ({
  ...
  // Calculate center area for content
  const centerStartRow = Math.floor((gridSize - centerRows) / 2);
  const centerEndRow = centerStartRow + centerRows;
  const centerStartCol = Math.floor((gridSize - centerCols) / 2);
  const centerEndCol = centerStartCol + centerCols;

  const isInCenterArea = (row: number, col: number) => {
    return row >= centerStartRow && row < centerEndRow && col >= centerStartCol && col < centerEndCol;
  };
  ...
  return (
    <div className="default-animation" style={wrapperStyle}>
      <div ref={sceneRef} className="default-animation--scene" style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => {
            // Skip cubes in center area for content
            if (centerContent && isInCenterArea(r, c)) {
              // Only render the center content once at the top-left of the center area
              if (r === centerStartRow && c === centerStartCol) {
                return (
                  <div
                    key={`center-${r}-${c}`}
                    style={{
                      gridRow: `${centerStartRow + 1} / span ${centerRows}`,
                      gridColumn: `${centerStartCol + 1} / span ${centerCols}`,
                      display: 'flex',
                      alignItems: 'center',
            ...
            return (
              <div key={`${r}-${c}`} data-row={r} data-col={c} className="cube">
